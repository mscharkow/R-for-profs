...
eurovision %>%
  group_by(to_country) %>%
  summarise(years = n_distinct(year), avg_points = mean(points, na.rm = T)) %>%
  arrange(-years, -avg_points)

d = eurovision %>%
  filter(to_country == "Spain" & semi_final == "f" & is.na(duplicate))
d
...
# Zeiteffekt
cor.test(~ points + year, data = d)
ct1 = cor.test(~ points + year, data = d)
report::report_text(ct1)

lm2 = lm(points ~ year, data = d)
summary(lm2)
report::report_table(lm2)

# Zeit und Televoting
lm3 = lm(points ~ jury_or_televoting + year, data = d)
report::report_table(lm3)

# Interaktionseffekte
lm4a = lm(points ~ jury_or_televoting + year + jury_or_televoting:year, data = d)
lm4b = lm(points ~ jury_or_televoting * year, data = d)
lm4a; lm4b
...
# Kontraste
marginaleffects::comparisons(lm1) %>%
  summary()
...
marginaleffects::predictions(lm4a) %>%
  ...
  geom_jitter(data = d, aes(x = year, y = points,
